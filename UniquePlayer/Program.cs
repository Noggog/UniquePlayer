...
{
    public class Program : APatcher<ISkyrimMod, ISkyrimModGetter, Settings>
    {
        ...
        public static async Task<int> Main(string[] args)
        {
            ...
            return await SynthesisPipeline.Instance
                ...
                .AddPatch<ISkyrimMod, ISkyrimModGetter, Program>()
                .SetAutogeneratedSettings<Program, Settings>()
...
&& !filePath.EndsWith(outfitOutputFileName)
        ...
        public override async Task RunPatch()
        {
            var outfitFilesTask = new Task(() => CopyAndModifyOutfitFiles(State.RunArguments.DataFolderPath));
            ...
            var texturesPath = State.RunArguments.DataFolderPath + "\\Textures\\";
            var meshesPath = State.RunArguments.DataFolderPath + "\\Meshes\\";
            ...
            foreach (var (raceLink, vampireRaceLink) in playableRaceFormLinks.Zip(playableVampireRaceFormLinks))
            {
                ...
                var vampireRace = vampireRaceLink.Resolve(State.LinkCache);
            ...
            foreach (var item in presetCharacters)
                State.PatchMod.Npcs.GetOrAddAsOverride(item.Resolve(State.LinkCache)).RemapLinks(replacementPlayableRacesDict);
            ...
            foreach (var armorAddon in State.LoadOrder.PriorityOrder.WinningOverrides<IArmorAddonGetter>().Where(x => victimRaceFormKeys.Contains(x.Race) || x.AdditionalRaces.Any(y => victimRaceFormKeys.Contains(y))).ToList())
            {
                ...
                void modelNeedsEdit(IModelGetter? model)
                {
                    ...
                    model.AlternateTextures?.ForEach(alternateTexture => needsEdit |= UpdateTextureSet(alternateTexture.NewTexture, texturesPath, ResolveOrThrow<ITextureSetGetter>(armorAddon), NewTextureSet()));
                }
                ...
                {
                    if (!x.IsNull) needsEdit |= UpdateTextureSet(x, texturesPath, ResolveOrThrow<ITextureSetGetter>(armorAddon), NewTextureSet());
                });
        ...
        public Race CopyRace(IRaceGetter oldRace, string texturesPath, string meshesPath)
        {
            ...
            {
                headData.FaceDetails.NotNull().ForEach(x => UpdateTextureSet(x, texturesPath, ResolveOrThrow<ITextureSetGetter>(race), NewTextureSet()));
        ...
        public void UpdateHeadPart(IFormLinkGetter<IHeadPartGetter> headPartItem, IMajorRecordCommonGetter race, string texturesPath, string meshesPath)
        {
            ...
            if (!headPart.TextureSet.IsNull)
                changed |= UpdateTextureSet(headPart.TextureSet, texturesPath, ResolveOrThrow<ITextureSetGetter>(headPart), NewTextureSet());
            ...
            {
                changed |= UpdateTextureSet(x.NewTexture, texturesPath, ResolveOrThrow<ITextureSetGetter>(headPart), NewTextureSet());
            });
        ...
        public Func<IFormLinkGetter<T>, Func<string>, T> ResolveOrThrow<T>(
        ...
        public Func<string, ITextureSet> NewTextureSet() => (string editorID) => State.PatchMod.TextureSets.AddNew(editorID);
